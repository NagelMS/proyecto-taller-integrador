...
Por último, el sistema contempla desde su diseño la escalabilidad y la interoperabilidad. El uso de protocolos establecidos (UART para GPS, SPI para LoRa, I2C para la OLED y sensores auxiliares) permite integrar otros dispositivos en el futuro, como sensores de actividad, temperatura o salud del animal, sin alterar la arquitectura base. Del lado del backend, la utilización de tramas normalizadas y almacenamiento estructurado de la información facilita la integración con aplicaciones web de terceros, paneles de visualización y herramientas analíticas orientadas a la toma de decisiones en manejo de ganado.
...
De manera similar, el código provee una gran cantidad de utiliarios \textit{wrappers} de bibliotecas de Arduino para la gestión de más recursos de la placa, un claro ejemplo de esto son las bibliotecas \textit{web\_utils.h}, \textit{wx\_utils.h} y \textit{wifi\_utils.h}
...
Inicialmente, en el dispositivo, se logró la configuración y operación del módulo LilyGo T-Beam v1.2 como tracker LoRa/APRS. El firmware permite obtener lecturas de posición a partir del receptor GPS, empaquetarlas en tramas APRS y transmitirlas a través del transceptor LoRa. Como resultado visible para el usuario, se actualizó el recurso gráfico desplegado en la pantalla OLED, sustituyendo el ícono genérico por uno asociado semánticamente a la aplicación de rastreo ganadero. Esta mejora, aunque es sencilla, contribuye a una interfaz más intuitiva y orientada al caso de uso final.
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=1\linewidth]{fig/menu.jpeg}
    \caption{Menu interno de configuración}
    \label{fig:menu}
\end{figure}
...
Finalmente, se realizaron ensayos de campo específicos para evaluar el comportamiento del smartbeaconing con telemetría de voltaje de batería activa. En estas pruebas el tracker se configuró para reportar tanto la posición como el nivel de batería, y se observó que, en condiciones de reposo, el intervalo entre beacons fue del orden de cuatro minutos, mientras que en movimiento se redujo aproximadamente a un rango entre un minuto y medio y dos minutos. No obstante, también se detectó que el cálculo de velocidad basado en las lecturas GPS puede presentar saltos bruscos de posición, como se observa en la Fig. \ref{fig:smart.png}, lo cual introduce cierta imprecisión en la lógica que decide cuándo aumentar la frecuencia de transmisión. 

En la Fig. \ref{fig:graf.jpeg} se observa el desempeño de la batería obtenido por medio de la telemetría, esto tambien comprueba el funcionamiento de apagado tras obtener una tensión de 3.0 V en la batería.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=1\linewidth]{fig/grafbat.jpeg}
...
\begin{itemize}
    \item El sistema de rastreo basado en la plataforma LilyGo T-Beam, en conjunto con LoRa y APRS, demostró ser una opción eficiente para el monitoreo de ganado en contextos rurales con limitada cobertura celular.
    \item Se logró establecer correctamente el flujo completo de información: adquisición de posición mediante GPS, empaquetado de datos en tramas APRS, transmisión por enlace LoRa y visualización posterior en plataformas de mapeo compatibles con APRS.
    \item La integración de telemetría de voltaje de batería dentro de la trama institucional permite no solo localizar al animal, sino también monitorear el estado energético del dispositivo, lo cual es clave para la operación prolongada en campo.
    \item El uso de \textit{smartbeaconing} permitió adaptar dinámicamente la frecuencia de envío de beacons según el movimiento, evidenciando reducción del intervalo de reporte en condiciones de desplazamiento y una menor frecuencia en reposo.
    \item A nivel regulatorio, el sistema opera dentro de las bandas y límites de potencia permitidos en Costa Rica, cumpliendo con las disposiciones del PNAF y las restricciones asociadas al servicio de radioaficionados.

\end{itemize}

\section{Recomendaciones}
\begin{itemize}
    \item Realizar campañas de prueba más extensas en escenarios rurales representativos (diferentes topografías, densidad de vegetación y distancias) para caracterizar con mayor detalle el alcance, la confiabilidad del enlace y el comportamiento del \textit{smartbeaconing}.
    \item Implementar filtros o técnicas de suavizado sobre los datos GPS para reducir los saltos bruscos de posición y mejorar el cálculo de velocidad, incrementando la precisión del algoritmo de \textit{smartbeaconing}.
    \item Incorporar registros sistemáticos de consumo energético (corriente y tensión) bajo distintos patrones de uso, con el objetivo de estimar de forma más precisa la autonomía y dimensionar adecuadamente la batería y posibles sistemas de recarga.
\end{itemize}
...
Acorde a los avances realizados en el proyecto y los resultados obtenidos, se cumplieron los tiempos establecidos. A continuación, se presenta el cronograma detallado de las actividades planificadas para la culminación del proyecto:
